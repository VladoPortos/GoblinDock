...
(function () {
  ...
  function App({ initialRoute }) {
    ...
    // Live state: an SSE "state changed" ping triggers a single /api/state refetch
    // (GDStore.refresh() coalesces concurrent calls). A slow 30s poll is a backstop
    // for dropped streams and out-of-band Proxmox changes. The builder ('newtemplate')
    // and the job screen ('job') manage their own data, so we don't refetch under them.
    useEffect(() => {
      if (route === 'login') return undefined;
      const quiet = () => route === 'newtemplate' || route === 'job';
      const refresh = () => { if (!quiet()) window.GDStore.refresh().catch(() => {}); };
      ...
      } catch (e) { /* SSE unavailable — safety poll still covers us */ }
